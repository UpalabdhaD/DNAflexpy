...
mkdocs serve                        # docs preview; mkdocs build --strict to render
python scripts/make_doc_fixtures.py /tmp/docfix   # inputs the doc examples open
MPLBACKEND=Agg python scripts/check_doc_examples.py /tmp/docfix   # runs them, in CI
...
`scripts/check_doc_examples.py` runs every ```` ```python ```` block in `docs/*.md` and `README.md`, sharing one namespace per file, **and compares printed output against the untagged ```` ``` ```` block that follows it**. That second check exists because the first is not enough: a block can run perfectly while the output shown underneath it was invented. That mistake has been made twice in this repo and caught both times only after the check was added. Output that genuinely cannot be reproduced belongs in a tagged fence (```` ```text ````) so it is skipped. Both this and `scripts/make_doc_fixtures.py` now run in the `test` CI job.

`DNAflexpy/data/examples/` ships five small files reached through `example_path()` / `example_files()`, the equivalent of DNAshapeR's `system.file("extdata", ...)`. They are generated by `scripts/make_example_data.py` and committed. The promoters carry an AT-rich element at bases 40–60 and `control.fa` is composition-matched without it, so the worked examples show a real signal; `tests/test_examples.py::test_the_promoters_carry_a_real_signal` asserts that signal is still detectable, because documentation whose figures show nothing teaches nothing.

**`pyfaidx` writes a `.fai` index next to the genome it reads.** Pointing `from_bed` at the packaged `example_path("genome.fa")` therefore tries to write into `site-packages`, which fails on a read-only install. The tutorial copies the example data to a writable directory first, and `.gitignore` covers `*.fa.fai`.

Citation metadata lives in `DNAflexpy/citation.py` and nothing else; `--citation` and `docs/citation.md` both read it, and the version comes from `__version__` rather than being written twice.
